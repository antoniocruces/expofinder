...
(function(jQuery) {
	jQuery.fn.attc = function(options) {
		...
		var settings = jQuery.extend({
			...
			'controlsLabels': {
				showChart: gcTXT.showChart,
				hideChart: gcTXT.hideChart,
				showTable: gcTXT.showTable,
				hideTable: gcTXT.hideTable,
				createChart: gcTXT.createChart,
				changeChart: gcTXT.changeChart,
			},
			chartOptionList: '<option value="bar">' + 
				gcTXT.bar + '</option><option value="pie">' + 
				gcTXT.pie + '</option><option value="column">' + 
				gcTXT.column + '</option><option value="area">' + 
				gcTXT.area + '</option><option value="line">' + 
				gcTXT.line + '</option>'
		}, options);
		...
		var CreateChart = function(type, location, tableEl) {
			...
			switch (type) {
				case 'pie':
					var headerValues = jQuery('#' + tableEl.attr('data-attc-colvalues'));
					var headerDesc = tableEl.attr('data-attc-colDescription');
					if (headerDesc.split(',').length > 0) {
						headerDesc = jQuery('#' + headerDesc.split(',')[0]);
					} else {
						headerDesc = jQuery('#' + headerDesc);
					}
				...
				case 'area':
				case 'line':
					var headerDesc = jQuery('#' + tableEl.attr('data-attc-colDescription'));
					var colIndexDesc = headerDesc.index() + 1;
					...
					for (x = 0; x < headerValuesArray.length; x++) {
						headerIndexArray.push(jQuery('#' + headerValuesArray[x]).index() + 1);
					}
		...
		if (tableEl.length > 0) {
			if (tableEl.get(0).tagName == 'TABLE') {
				...
				if (controls != '') {
					...
					var controlsEl = jQuery(controls);
					...
					controlsEl.on("click", "a[data-attc-controls-showHide]", function(e) {
						e.preventDefault();
						var el = jQuery('#' + jQuery(this).attr('data-attc-controls-showHide'));
						el.find('tbody').show();
						el.toggle();
						jQuery(this).text(el.is(':visible') ? jQuery(this).attr('data-attc-textHide') : jQuery(this).attr('data-attc-textShow'));
					});
					controlsEl.on("change", "select[data-attc-controls-updateChart]", function() {
						//update chart type for other functions
						tableEl.attr('data-attc-type', jQuery(this).val());
						//grab the new chart and re-run the creation
						CreateChart(jQuery(this).val(), settings.location, tableEl);
					...
					controlsEl.find("a[data-attc-controls-addCreateOptions]").on("click", function(e) {
						...
						tableEl.find('thead th').each(function(index) {
							...
							tableEl.find('tbody>tr>td:nth-child(' + col + ')').each(function() {
								items += parseFloat(jQuery(this).text().replace(numPattern, ''));
							});
							...
							if (jQuery.inArray(jQuery(this).attr('id'), valuesCols) > -1) {
								appendEl += ' checked="checked"';
							}
							appendEl += ' class="attcEditCheckRadioBoxes"/>';
							appendEl = jQuery(appendEl).on("click", function() {
								//give this col an id if it doesn't have one
								var myId = 'attcCol' + jQuery(this).val();
								if (jQuery(this).parents('th').attr('id') != undefined) {
									myId = jQuery(this).parents('th').attr('id');
								} else {
									jQuery(this).parents('th').attr('id', myId);
								}
								//change the attribute to this column
								if (jQuery(this).attr('data-attc-colType') == 'desc') {
									...
									if (tableEl.attr('data-attc-type') == 'pie') {
										...
										var checkedIds = tableEl.find('input[data-attc-colType=value]:checked').parents('th').map(function() {
											return this.id;
										}).get().toString();
...
jQuery(document).ready(function() {
	jQuery('[data-attc-createChart]').attc();
