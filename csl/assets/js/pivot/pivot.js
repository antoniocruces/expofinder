(function() {
	...
	callWithJQuery(function($) {
		...
		pivotTableRenderer = function(pivotData, opts) {
			...
			result.className = "table table-hover table-condensed table-bordered";
			result.setAttribute("id", "pvtTblResults");
			...
			tz = document.createElement("thead");
			for (j in colAttrs) {
			...
			tz = document.createElement("tbody");
			for (i in rowKeys) {
				...
				for (j in colKeys) {
					...
					td.className = "pvtVal row" + i + " col" + j + (!isNaN(parseFloat(val)) && isFinite(val) ? " text-right" : " text-left");
				...
				td = document.createElement("td");
				td.className = "pvtTotal rowTotal text-right";
			...
			for (j in colKeys) {
				...
				val = totalAggregator.value();
				td = document.createElement("td");
				td.className = "pvtTotal colTotal text-right";
				td.textContent = totalAggregator.format(val);
				td.setAttribute("data-value", val);
			...
			val = totalAggregator.value();
			td = document.createElement("td");
			td.className = "pvtGrandTotal text-right";
			td.textContent = totalAggregator.format(val);
			td.setAttribute("data-value", val);
			tr.appendChild(td);
			result.appendChild(tz);
		...
		$.fn.pivotUI = function(input, inputOpts, overwrite, locale) {
			...
			try {
				...
				//tbContainer.setAttribute("id", "pvtTBContainer");
				rendererControl = $("<div>").addClass('classREND');
				...
				colList = $("<ul>").addClass('pvtAxisContainer pvtUnused');
				...
				fn = function(c) {
					var 
						attrElem, btns, checkContainer, 
						filterItem, filterItemExcluded, hasExcludedItem, 
						keys, len3, o, 
						ref2, showFilterList, 
						triangleLink, updateFilter, v, 
						valueList;
					keys = (function() {
						var results;
						results = [];
					...
					valueList = $("<div class=\"classFILT\">").addClass('pvtFilterBox').hide();
					valueList.append($("<h4>").text(c + " (" + keys.length + ")"));
					if (keys.length > opts.menuLimit) {
						valueList.append($("<p>").html(opts.localeStrings.tooMany));
					} else {
						btns = $("<p>").appendTo(valueList);
						btns.append($("<button class=\"btn btn-success\">", {
							type: "button"
						}).html(opts.localeStrings.selectAll).bind("click", function() {
							return valueList.find("input:visible").prop("checked", true);
						}));
						btns.append($("<button class=\"btn btn-danger\">", {
							type: "button"
					...
					$("<p>").appendTo(valueList).append($("<button class=\"btn btn-default\">", {
						type: "button"
					}).text(opts.localeStrings.lblOK).bind("click", updateFilter));
					...
					triangleLink = $("<span>").addClass('pvtTriangle glyphicon glyphicon-filter').bind("click", showFilterList);
					attrElem = $("<li>").addClass("axis_" + i).append($("<span>").addClass('pvtAttr btn btn-default btn-sm').text(c).data("attrName", c).append(triangleLink));
					if (hasExcludedItem) {
						attrElem.addClass('pvtFilteredAttribute');
					}
				...
				tr1 = $("<div class=\"classTR1 col-md-4\">").appendTo(uiContainer);
				...
				$("<div>").addClass('pvtVals').appendTo(tr1).append(aggregator); 
				tr2 = $("<div class=\"classTR2 col-md-4\">").appendTo(uiContainer);
				tpCOLP = $("<div class=\"panel panel-info\">").appendTo(tr2);
				tpCOLH = $("<div class=\"panel-heading\">").appendTo(tpCOLP);
				tpCOLB = $("<div class=\"panel-body\">").appendTo(tpCOLP);
				tpCOLH.html('<span class="glyphicon glyphicon-option-vertical" style="margin-right: 5px;"></span>' + opts.localeStrings.lblCols);
				tpROWP = $("<div class=\"panel panel-warning\">").appendTo(tr2);
				tpROWH = $("<div class=\"panel-heading\">").appendTo(tpROWP);
				tpROWB = $("<div class=\"panel-body\">").appendTo(tpROWP);
				tpROWH.html('<span class="glyphicon glyphicon-option-horizontal" style="margin-right: 5px;"></span>' + opts.localeStrings.lblRows);
				$("<div class=\"classCOLS\">").addClass('pvtAxisContainer pvtHorizList pvtCols').appendTo(tpCOLB);
				tpROWB.append($("<div class=\"classCOLS\">").addClass('pvtAxisContainer pvtHorizList pvtRows').attr("valign", "top"));
				tr3 = $("<div class=\"classTR3 col-md-12\" style=\"overflow-x: auto; width: 100%;\">").appendTo(tbContainer);
				pivotTable = $("<div class=\"classREND col-md-12\">").attr("valign", "top").addClass('pvtRendererArea').appendTo(tr3);
				
				tr1.prepend(rendererControl);
				btPRNB = $("<button type=\"button\" id=\"prnTBL\" class=\"btn btn-default\">").appendTo(tr1).bind("click", function() {
					var divToPrint=document.getElementById("pvtTBContainer");
					newWin = window.open("");
					newWin.document.write( "<!DOCTYPE html>" );
					newWin.document.write( "<html>" );
					newWin.document.write( "<head>" );
					newWin.document.write( "<title>" );
					newWin.document.write( document.title );
					newWin.document.write( "</title>" );
					newWin.document.write( "<link rel='stylesheet' href='https://maxcdn.bootstrapcdn.com/bootstrap/3.3.6/css/bootstrap.min.css?ver=4.4' type='text/css' media='all' />" );
					newWin.document.write( "</head>" );
				...
				btPRNB.html('<span class="glyphicon glyphicon-print" style="margin-right: 5px;"></span>' + opts.localeStrings.lblPrint);

				tmpFilename = Math.random().toString(36).substring(7);
				btXLSB = $("<button type=\"button\" id=\"xlsTBL\" style=\"margin-left: 5px;\" class=\"btn btn-default\" download=\"" + tmpFilename + ".xls\">").appendTo(tr1).bind("click", function() {
					tableToExcel('pvtTBContainer', 'ExpoFinder');
				});
				btXLSB.html('<span class="glyphicon glyphicon-export" style="margin-right: 5px;"></span>' + opts.localeStrings.lblExcel);

				uiContainer.prepend($("<div class=\"classTR0 col-md-4\">").append(colList));
				
				this.html(uiContainer);
				this.append(tbContainer);
				ref3 = opts.cols;
